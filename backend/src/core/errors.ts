...
const createError = (code: ErrorCodes, message: string, statusCode?: number): AppErrorConstructor =>
  fastifyCreateError(code, message, statusCode)

const errors: Errors = {
  GENERIC_DB_QUERY_FAILED: createError(ErrorCodes.GENERIC_DB_QUERY_FAILED, '%s', 500),
